...
bool lsm6dsox_init() {
    // Configure CS as output and deassert immediately
    pinMode(LSM6DSOX_CS_PIN, OUTPUT);
    digitalWrite(LSM6DSOX_CS_PIN, HIGH);

    // Short settling delay after CS is driven high
    delay(10);
    ...
    // 2. Enable register auto-increment (IF_INC).
    //    Required for multi-byte burst reads — without it the sensor
    //    would return the same register address on every byte.
    write_register(LSM6DSOX_REG_CTRL3_C, LSM6DSOX_IF_INC);

    // 3. Configure accelerometer: 104 Hz, ±4 g
    write_register(LSM6DSOX_REG_CTRL1_XL, LSM6DSOX_XL_104HZ_4G);

    // 4. Configure gyroscope: 104 Hz, ±2000 dps
    write_register(LSM6DSOX_REG_CTRL2_G, LSM6DSOX_G_104HZ_2000DPS);
